...
def distance_of_text(vector1, vector2):
...
def build_text_distance_matrix(project):
    filename = 'E:/data/clustering/text_feature_vector' + '-' + project + '.txt'
    feature_vector = np.loadtxt(filename, delimiter=' ', dtype=float)
    ...
    for i in range(casecount):
        for j in range(i, casecount):
            DT = distance_of_text(feature_vector[i], feature_vector[j])
    ...
    DT_filename = 'E:/data/clustering/DTMatrix-' + project + '.txt'
...
def build_shot_distance_matrix(project, γ):
    ...
    DS_filename = 'E:/data/clustering/DSMatrix-' + project + '.txt'
...
def build_balanced_distance_matrix(project, same_shot_distance, balanced_factor):
    filename = 'E:/data/clustering/DTMatrix-' + project + '.txt'
    text_Matrix = np.loadtxt(filename, delimiter=' ', dtype=float)
    caselist = load_caselist(project)
    casecount = len(caselist)
    imagenamelist, SPM_Matrix = read_SPDistanceMatrix(project)
    ...
    BD_filename = 'E:/data/clustering/BDMatrix-' + project + '.txt'
